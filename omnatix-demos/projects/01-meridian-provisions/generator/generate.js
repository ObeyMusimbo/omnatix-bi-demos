...
const WAREHOUSES = [
  { code: 'WH-01', name: 'Meridian Central DC', region: 'Central' },
  { code: 'WH-02', name: 'Meridian Northern DC', region: 'North' },
  { code: 'WH-03', name: 'Meridian Coastal DC', region: 'Coastal' },
];

const REGIONS = ['Central', 'North', 'South', 'East', 'Coastal'];
const FREIGHT_RATE = { Central: 0.11, North: 0.15, South: 0.16, East: 0.17, Coastal: 0.14 };
const DROP_FEE = { Central: 5.5, North: 7.0, South: 7.5, East: 8.0, Coastal: 6.5 };
...
for (const [brand, category, subcat, variants] of LINES) {
  for (const [packLabel, kg, cost, list] of variants) {
    ...
    if (['Meadowvale Full Cream Milk 6x1L', 'Golden Acre Maize Meal 10kg', 'Softleaf Toilet Tissue 9pk x6']
...
const FIRST = ['Tendai', 'Naledi', 'Farai', 'Amara', 'Sipho', 'Chipo', 'Kwame', 'Zanele', 'Rudo', 'Thabo', 'Nomsa', 'Bongani', 'Lerato', 'Tafara', 'Anesu', 'Musa'];
const LAST = ['Moyo', 'Dube', 'Nkomo', 'Chirwa', 'Mensah', 'Banda', 'Sibanda', 'Okonkwo', 'Mwangi', 'Zulu', 'Phiri', 'Ncube', 'Osei', 'Kamau', 'Maseko'];
...
const MT_NAMES = ['Freshmark', 'Cityline Foods', 'Greenbay Super', 'Unity Grocer', 'Parkview Market', 'Eastgate Foods', 'Silverleaf Stores', 'Harbour Fresh'];
const WS_NAMES = ['Summit Cash & Carry', 'Bulkline Traders', 'Riverstone Wholesale', 'Apex Depot', 'Ironbridge Supply'];
const GT_NAMES = ['Corner Shop', 'Mini Mart', 'Family Store', 'Quick Stop', 'Market Stall', 'Village Grocer', 'Daily Needs', 'Roadside Store'];
const HR_NAMES = ['Lodge', 'Bistro', 'Cafe', 'Grill House', 'Hotel', 'Canteen', 'Takeaway'];
const CITY = ['Ridgeway', 'Marlowe', 'Kestrel Bay', 'Ashford', 'Northvale', 'Portmere', 'Highfield', 'Lakeside', 'Stonebridge', 'Fairmount'];
...
for (let d = 0; d < N_DAYS; d++) {
  ...
  for (let o = 0; o < nOrders; o++) {
    ...
    const wh = cust.region === 'North' ? WAREHOUSES[1] : cust.region === 'Coastal' ? WAREHOUSES[2] : WAREHOUSES[0];
    ...
    costs.push({
      ...
      handling_cost: r2(1.85 * pending.length + 3.2),
...
const key = `# Meridian Provisions Co. - demo answer key

Generated ${iso(Date.now())} from seed ${SEED}. Period ${iso(START)} to ${iso(END)}.
Trailing twelve months (TTM) = ${iso(TTM_START)} to ${iso(END)}.
...
|---|---|---|---|
| Revenue | ${m(ttmRev)} | ${m(pyRev)} | ${pct(ttmRev / pyRev - 1)} |
| Gross profit | ${m(ttmRev - ttmCogs)} | ${m(pyRev - pyCogs)} | ${pct((ttmRev - ttmCogs) / (pyRev - pyCogs) - 1)} |
| Gross margin | ${pct((ttmRev - ttmCogs) / ttmRev)} | ${pct((pyRev - pyCogs) / pyRev)} | ${((ttmRev - ttmCogs) / ttmRev - (pyRev - pyCogs) / pyRev) * 100 >= 0 ? '+' : ''}${(((ttmRev - ttmCogs) / ttmRev - (pyRev - pyCogs) / pyRev) * 100).toFixed(1)} pts |
...
|---|---|
| Revenue | ${m(p1Rev)} |
| Gross profit | ${m(p1Gp)} (${pct(p1Gp / p1Rev)}) |
| Allocated freight | ${m(p1Freight)} |
| Allocated rebates | ${m(p1Rebate)} |
| **Net contribution** | **${m(p1Gp - p1Freight - p1Rebate)}** |
...
|---|---|---|---|---|
| On promotion | ${p2Units.toLocaleString('en-US')} | ${m(p2Rev)} | ${m(p2Gp)} | ${m2(p2Gp / Math.max(1, p2Units))} |
| Off promotion | ${p2BaseUnits.toLocaleString('en-US')} | ${m(p2BaseRev)} | ${m(p2BaseGp)} | ${m2(p2BaseGp / Math.max(1, p2BaseUnits))} |

Gross profit forgone on promoted units, valued at the off-promotion rate:
**${m(p2BaseGp / Math.max(1, p2BaseUnits) * p2Units - p2Gp)}**
...
| Measure | TTM |
|---|---|
| Revenue | ${m(sumRev)} (${pct(sumRev / ttmRev)} of company) |
| Revenue forgone vs holding 8% | **${m(sumDiscLost)}** |
...
(${P4_SKUS.map((s) => bySku.get(s).product_name).join(', ')}) hit zero on hand on Thursdays in
about 35% of weeks and cannot be supplied Friday or Saturday.

| Measure | Full period |
|---|---|
| Units not supplied | ${ledger.lostUnits.toLocaleString('en-US')} |
| Estimated revenue forgone | **${m(ledger.lostRevenue)}** |
...
${P5_SKUS.size} SKUs still sitting in WH-02, valued at cost: **${m(deadValue)}**.
...
fs.writeFileSync(path.join(__dirname, '..', 'ANSWER_KEY.md'), key);
...
console.log(`\n  Finding 1 bulk water rev ${m(p1Rev)}  GP ${pct(p1Gp / p1Rev)}  contribution  ${m(p1Gp - p1Freight - p1Rebate)}`);
console.log(`  Finding 2 promo GP forgone         ${m(p2BaseGp / Math.max(1, p2BaseUnits) * p2Units - p2Gp)}`);
console.log(`  Finding 3 discount creep           ${m(sumDiscLost)}`);
console.log(`  Finding 4 stock-out revenue lost   ${m(ledger.lostRevenue)}`);
console.log(`  Finding 5 dead stock at cost       ${m(deadValue)}`);
console.log('\n  Answer key written to ANSWER_KEY.md');
